...
<div class="space-y-6 sm:space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 bg-white p-5 sm:p-6 rounded-2xl sm:rounded-3xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#0f172a]">
        <div>
            <h1 class="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2.5 sm:gap-3">
                <span class="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-pink-500 rounded-full inline-block border border-slate-900"></span>
                Kelola Tautan
            </h1>
        ...
        class="w-full sm:w-auto bg-pink-300 hover:bg-pink-200 text-slate-950 font-extrabold py-2.5 sm:py-3 px-6 rounded-xl sm:rounded-2xl border-2 border-slate-900 shadow-[3px_3px_0px_0px_#0f172a] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none transition-all flex items-center justify-center gap-2">
    ...
    <div class="bg-white rounded-2xl sm:rounded-3xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#0f172a] sm:shadow-[6px_6px_0px_0px_#0f172a] overflow-hidden flex flex-col">
        ...
        <div class="hidden lg:grid grid-cols-12 gap-4 bg-pink-50 text-slate-900 px-6 py-4 border-b-2 border-slate-900 text-xs font-black uppercase tracking-wider">
        ...
        <div class="divide-y-2 divide-slate-900 bg-white">
            @forelse($links as $link)
                <div class="flex flex-col lg:grid lg:grid-cols-12 gap-4 lg:gap-4 items-start lg:items-center p-4 sm:p-6 lg:p-6 hover:bg-pink-50/60 transition-colors group">
                    ...
                    <div class="lg:col-span-5 flex items-center space-x-3 sm:space-x-4 w-full">
                        ...
                        <div class="overflow-hidden">
                            <div class="text-sm sm:text-base font-extrabold text-slate-900 group-hover:text-pink-500 transition-colors truncate">{{ $link->title }}</div>
                    ...
                    <div class="flex flex-row lg:contents w-full gap-4 mt-2 lg:mt-0">
                        ...
                        <div class="lg:col-span-3 flex flex-col lg:flex-row items-start lg:items-center flex-1">
                            ...
                            <div class="inline-flex items-center px-2.5 py-1 rounded-md sm:rounded-lg bg-pink-50 border border-slate-300 text-xs font-extrabold text-slate-800 whitespace-nowrap">
                                <i data-lucide="mouse-pointer-click" class="w-3.5 h-3.5 mr-1.5 text-pink-500"></i>
       ...
       class="flex-1 lg:flex-none text-center px-4 py-2 bg-fuchsia-200 text-slate-900 rounded-lg sm:rounded-xl border-2 border-slate-900 shadow-[2px_2px_0px_0px_#0f172a] hover:bg-fuchsia-300 text-xs font-bold transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none">
        Edit
    </a>
        ...
        <button type="submit"
            ...
            @empty
                ...
                <div class="px-6 py-16 text-center">
                    <div class="flex flex-col items-center justify-center max-w-sm mx-auto p-6 bg-pink-50 rounded-2xl border-2 border-dashed border-slate-400">
                        <div class="bg-pink-200 p-3 rounded-2xl border-2 border-slate-900 shadow-[2px_2px_0px_0px_#0f172a] mb-3 text-slate-900">
        ...
        <!-- Pagination Section -->
        @if($links->hasPages())
            <div class="bg-pink-50 border-t-2 border-slate-900 px-6 py-4">
